...
def compute_geohash_key(geoh, with_neighbors=True, radius_km=None):
    ...
    if with_neighbors:
        if radius_km is not None and radius_km > 0:
            ...
            layers = _calculate_geohash_layers(radius_km, len(geoh))
            neighbors = _expand_geohash_layers(geoh, layers)
        else:
...
def _calculate_geohash_layers(radius_km, precision):
...
def _expand_geohash_layers(geoh, layers=1):
